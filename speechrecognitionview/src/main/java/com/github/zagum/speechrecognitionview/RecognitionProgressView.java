...
public class RecognitionProgressView extends View implements RecognitionListener {
     ...
     *
     * @param colors - array with size = {@link #barCount}
     */
    public void setColors(int[] colors) {
        if (colors == null) return;

        barColors = new int[barCount];
        if (colors.length < barCount) {
            System.arraycopy(colors, 0, barColors, 0, colors.length);
            for (int i = colors.length; i < barCount; i++) {
                barColors[i] = colors[0];
            }
        } else {
            System.arraycopy(colors, 0, barColors, 0, barCount);
        }
    }
    ...
    protected void initBars() {
        ...
        for (int i = 0; i < barCount; i++) {
            int sign = i % 2 == 0? -1 : 1;
            int x = firstCirclePosition + (2 * radius + spacing) * ((i+1)/2) * sign;
